...
- one additive, source-only count-repair QA call preserves verified items and requests only the missing uncited concepts on content-rich sources; a remaining mismatch fails with `qa_item_count_mismatch`, while wholly unsupported output fails with `ungrounded_generation_output` without retrying the full job
