...
- up to three bounded additive, source-only count-repair QA calls preserve verified items, request only the missing concepts, and provide a shortlist of still-unused source statements; repair stops immediately if an attempt adds no verified item, a remaining mismatch fails with `qa_item_count_mismatch`, and wholly unsupported output fails with `ungrounded_generation_output` without retrying the full job
